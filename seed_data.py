"""Наполнение базы данных тестовыми данными (без удаления существующих таблиц)."""
from models import db, Category, Component, GameFPS

# Все ваши данные из seed.py
...
def fill_data():
    ...
    for game, gpu_name, cpu_name, fps in GAME_FPS:
        if gpu_name in comps and cpu_name in comps:
            db.session.add(GameFPS(
                game=game,
                gpu_id=comps[gpu_name].id,
                cpu_id=comps[cpu_name].id,
                fps=fps,
            ))
    db.session.commit()

    print(f"✅ Загружено: {len(CATEGORIES)} категорий, {len(COMPONENTS)} компонентов, {len(GAME_FPS)} записей FPS.")